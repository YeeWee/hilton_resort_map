...
把 Hilton 官方 [Resort Credit Eligible Hotels](https://www.hilton.com/en/p/hilton-honors/resort-credit-eligible-hotels/) 页面列出的酒店,按品牌呈现在一张 Leaflet 地图上。数据全集就是该页面列出的全部酒店,不多不少;页面顶部显示"共 N 家 / M 个品牌"供与官方页面核对。术语见 `CONTEXT.md`。
...
src/                     管线核心:页面解析、地理编码(回落链)、overrides 合并
...
1. **管线整体**:保存的页面 HTML fixture + 注入的假地理编码器 → 断言 `data/hotels.json` 契约(品牌齐全、hotel code 去重、Region 合并、总数守恒、失败显式标记)。
2. **页面整体**:headless Chrome 加载页面 + fixture 版 hotels.json → 断言用户可见的 DOM 行为(marker 总数、图例筛选、聚合展开、popup 字段、侧边栏定位、统计行)。CDN 与底图在路由层由本地副本应答。
...
| `region` | string \| null | Region,取自 by-region tab |
...
页面侧约定:页面只消费本文件,对 `brand` 为 null 的酒店灰点展示、图例与侧边栏置底;`lat` / `lng` 为 null 的酒店不生成 marker。
